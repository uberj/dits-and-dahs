package com.example.uberj.test1.views;
...
public class FormattingSeekBarPreference extends Preference {
    ...
    private SeekBar.OnSeekBarChangeListener mSeekBarChangeListener = new SeekBar.OnSeekBarChangeListener() {
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            if (fromUser && (mUpdatesContinuously || !mTrackingTouch)) {
                ...
                updateLabelValue(normalize(progress) + mMin);
            }
        }
        ...
        public void onStopTrackingTouch(SeekBar seekBar) {
            ...
            seekBar.setProgress(normalize(seekBar.getProgress()));
            if (seekBar.getProgress() + mMin != mSeekBarValue) {
                syncValueInternal(seekBar);
            }
        }

        private int normalize(int progress) {
            if (stepValue == 1) {
                return progress;
            }
            progress = progress / stepValue;
            return progress * stepValue;
        }
    };
    ...
    public FormattingSeekBarPreference(
    ...
    public FormattingSeekBarPreference(Context context, AttributeSet attrs) {
    ...
    public FormattingSeekBarPreference(Context context) {
        this(context, null);
    }
    ...
    void updateLabelValue(int value) {
        if (mSeekBarValueTextView != null) {
            mSeekBarValueTextView.setText(formatValue(value));
        }
    }
